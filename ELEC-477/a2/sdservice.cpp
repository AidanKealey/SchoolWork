...
void SDService::start() {
    ...
    while(alive) {
        ...
        if(n > 0) {
            cerr << "SDService::start() successful recieved in while" << endl;
        } else {
            cerr << "SDService::start() bad recieved in while" << endl;
        }
...
void SDService::queryDirectory(E477DirectoryService::DSRequest &receivedMsg, E477DirectoryService::DSResponse &replyMsg) {
    ...
    if(receivedMsg.has_registerserviceargs()) {
        cerr << "registering new server in directory" << endl;
        ...
        if(registerStatus) {
            cerr << "registered new server -- GOOD" << endl;
        } else {
            cerr << "registered new server -- FAILED" << endl;
        }
    ...
    if(receivedMsg.has_deleteserviceargs()) {
        cerr << "deleting server in directory" << endl;
