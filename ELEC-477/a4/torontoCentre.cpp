        ...
        if (samples.length() > 0){
            ...
            for (sample_iter = samples.begin(); sample_iter < samples.end(); ++sample_iter) {
                ...
                if (info.valid()) {
                    ...
                    cout << "**** subTorontoCentre received: " << ++count << endl;
                    ...
                    if ((distance <= 8.5 && distance >= 8) && flightHeightft <=3000) {
                        if (planeInBoundaryBuffer.find(msg.callsign()) == planeInBoundaryBuffer.end()) {
                            ...
                            // send message to AD
                            Radar::Route alert(msg.callsign(), Radar::zone::centre, Radar::zone::AD, msg.timestamp());
                            writer.write(alert);
                        }
                        ...
                        if (msg.vertrate() < 0){
                            if (planeInBoundaryBuffer.find(msg.callsign()) == planeInBoundaryBuffer.end()) {
                                planeInBoundaryBuffer.insert(msg.callsign());
                                cout << "\n----TorontoCentre Boundary Alert---- \n"
                                 << "\nAircraft entering Toronto AD by descending through 3000ft\n"
                                 << "  |  Flight : \"" << msg.callsign() << "\" \n" 
                                 << "  |  Distance from airport : \"" << distance << "nm\" \n" 
                                 << "  |  The plane is heading " << direction << " the airport \n"
                                 << "  |  Geo Altitude : \"" << flightHeightft << "ft\" \n"
                    ...
                    // remove from set
                    if (distance <= 8 && flightHeightft <= 3000 && (planeInBoundaryBuffer.find(msg.callsign()) != planeInBoundaryBuffer.end())) {
