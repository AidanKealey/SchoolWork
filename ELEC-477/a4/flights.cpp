...
int main(int argc, char * argv[])
{
    ...
    const string fileName = "../dataSorted.csv";
    // const string fileName = "../TorontoAll.csv";
    ...
    // wait for torontoAD before we start the loop.
    cout << programName << ": waiting for torontoAD to start" << endl;
    dds::core::cond::WaitSet waitset;
    dds::core::cond::StatusCondition wsc(writer);
    wsc.enabled_statuses(dds::core::status::StatusMask::publication_matched());
    waitset.attach_condition(wsc);
    try{
        ...
        cerr << programName << ": encountered an exception while waiting for torontoAD: \"" << e.what() << "\"" << endl;
        exit(1);
    }
    ...
    while(file >> aircraft)
    {
        ...
        if (aircraft.time > lastUsedTime){
            ...
            // for fast testing
            std::this_thread::sleep_for(std::chrono::milliseconds(100)); // --- change this back
            // std::this_thread::sleep_for(std::chrono::seconds(aircraft.time - lastUsedTime)); // --- change this back
